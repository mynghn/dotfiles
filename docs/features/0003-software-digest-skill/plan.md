...
3. 픽스처 3종 핀 고정 — 외부 접근 최소·재현 가능 기준으로 선정:
   - **델타형**: 공개 OSS의 머지된 PR 1개 — 설명과 링크된 이슈가 충실한 것.
     이슈·PR 본문은 스냅샷으로 동봉, 머지 SHA 핀.
   - **전체형**: 이 chezmoi 레포 자체(로컬 실물, 사용자가 직접 검증 가능).
     SHA 핀.
   - **외부형**: 낯선 중형(대략 5k–50k LOC) 공개 OSS 레포, 태그 핀.
   - (사내 레포/PR로 교체하고 싶으면 사용자가 이 항목을 수정한다.)
