...
ANCHOR_RE = re.compile(r"^(#{2,4})\s+((AC|INV|Decision)-(\d+):\s+([a-z0-9][a-z0-9-]*))\s*$", re.MULTILINE)
...
CITATION_RE = re.compile(
    ...
    r"(?:AC|INV|Decision)-\d+-[a-z0-9][a-z0-9-]*|Task:[A-Za-z][A-Za-z0-9-]*)"
)
...
class Validator:
    def __init__(self, feature_dir: Path, stage: str, allow_large: bool) -> None:
    ...
    def _check_spec(self) -> None:
        ...
        acs = self._anchors("spec", kind="AC")
        if not acs:
            self._error("spec.md", "SPEC Outcome must contain at least one AC anchor")
    ...
    def _check_plan(self) -> None:
        ...
        if not text:
            return
        self._require_sections("plan.md", text, ("Dependency DAG",))
        if "```mermaid" not in self._section_body(text, "Dependency DAG"):
            self._error("plan.md", "Dependency DAG must be Mermaid")
        self._warn_ascii_diagram("plan.md", text)
        tasks = self._tasks()
        if not tasks:
            self._error("plan.md", "TASK artifact must contain at least one task card")
        if len(tasks) > 16 and not self.allow_large:
            self._error("plan.md", f"task count {len(tasks)} exceeds one-deployment guardrail")
        elif len(tasks) > 12:
            self._warn("plan.md", f"task count {len(tasks)} is near the one-deployment limit")
        if re.search(r"^\s*-\s+\[[ xX]\]", text, re.MULTILINE):
    ...
    def _check_traceability(self) -> None:
        ...
        for anchor in self._anchors("spec", kind="AC") + self._anchors("spec", kind="INV"):
            target = anchor["target"]
            if f"SPEC#{target}" not in plan_text:
...
def main(argv: list[str]) -> int:
    ...
    issues = Validator(args.feature_dir, args.stage, args.allow_large).run()
    if args.json:
