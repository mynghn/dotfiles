...
or the narrower version, that it appears *only* in one form, in one place, for one purpose — search
the source for it, documentation included. Documentation is where a project most often answers the
question you assumed only its dependency could, and it is the half of a repository that a reader of
the code alone forgets to search. An edge drawn in the wrong place is worse than no edge: it sends
the reader away from an answer that was sitting there.
