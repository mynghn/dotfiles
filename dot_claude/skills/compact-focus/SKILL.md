...
Produce a single ready-to-paste `/compact <focus>` command line that compacts THIS conversation well, following the `<compaction>` policy in the global instructions (load `~/.local/share/metacognition-vault/context-engineering/knowledge/compaction-vs-eviction.md` and its siblings if you need the full reasoning).
