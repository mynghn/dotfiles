---
name: plan
description: LeanPlan — produce a TASK DAG from a DESIGN. Track subgraphs, prefixed task IDs, bidirectional verification against SPEC + DESIGN, one-deployment guardrail.
...
- Inline anchors format: `SPEC#AC-<N>-<slug>`, `DESIGN#Decision-<N>-<slug>`, doc-Guideline reference.
...
- **Anchors carry ID + slug (identity, not restatement).** `SPEC#AC-1-detected-anomaly-published-within-5s` — ID stable across slug edits, slug names the reference at-a-glance. Do not paraphrase the AC's content in the task card; rely on the anchor + JIT load when needed.
...
4. **Author `## Task: <id>` cards**, each with:
   ...
   - **Completion** — observable verification + method when non-obvious. Split dev / prod for infra + DB. Continuous Invariants → monitor / SLO / CI gate; episodic ACs → one-shot test.
...
6. **Bidirectional verification** — do this explicitly and report. Do not paper over gaps:
   - **Forward**: walk every SPEC `## AC-<N>` and every Invariant; each must map to ≥ 1 task Completion criterion. List uncovered items.
   - **Reverse**: walk every `## Task: <id>`; each Goal must cite ≥ 1 SPEC AC / Invariant / DESIGN Decision / doc Guideline as its reason. List orphan tasks.
7. **One-deployment guardrail** — if the DAG trips any of these, pause and surface a split question to the user before continuing:
   - > ~15 tasks.
   - > 3 repos touched.
   - > 2 parallel rollout tracks.
   Splitting an oversized plan is cheaper than shipping one that blows its scope.
8. **Self-check**:
...
- One-deployment guardrail satisfied (or split prompt raised and resolved).
