...
| Section | Required? |
|---|---|
| Outcome (tech future state + episode-verifiable conditions) | yes |
| Invariants (continuous: SLA, non-blocking, integrity) | conditional |
| Non-goals | conditional — tech-level scope ambiguity |
...
| Verification mapping (bidirectional) — every SPEC Outcome AC and Invariant maps to ≥ 1 TASK completion criterion; every TASK cites ≥ 1 SPEC AC, Invariant, DESIGN decision, or Guideline as its reason | Catches both unverified requirements (SPEC → TASK gap) and orphan implementation work (TASK without plan justification) |
...
| One-deployment guardrail | skill-prompt check at TASK time (DAG-size threshold) as final safety net; earlier-stage heuristics (SPEC AC count, DESIGN component count) would catch oversize sooner but are deferred — see open items |
...
| SPEC Outcome ↔ Invariants | Episode-triggered condition → Outcome. Continuous property that must hold regardless of implementation — including environmental bindings (existing backbone, compliance boundary, deployment envelope) — → Invariants. Chosen realization with real alternatives → DESIGN. If no alternative existed, it is not a choice — push back up to Invariants (avoids false optionality in DESIGN). |
...
| JIT anchor heading patterns | `## AC-<N>: <slug>`, `## Decision-<N>: <slug>`, `## Task: <id>` (track-prefixed like P1 / A1 / D1 / I1) | ID enables stable cross-reference across slug edits; slug carries identity inline so JIT load isn't forced just to see what the reference is |
...
- **AC split: episode vs. continuous.** Traditional "Acceptance Criteria" conflates episode-verifiable ("when X, Y happens") with continuous constraints ("p99 < 5s"). Split: former → Outcome; latter → Invariants. Different observability downstream (test vs. SLO dashboard).
- **References carry ID + slug (identity, not restatement).** Anchors look like `SPEC#AC-1-detected-anomaly-published-within-5s`. ID enables stable citation across slug edits; slug names the reference at-a-glance so agents and humans can orient without JIT-loading every hop. This is the reference's *identity*, not a restatement of its *content* (the AC's conditions and constraints). Agent still JIT-loads full content when needed. At the code-migration boundary (principle 8), distill semantic content into commit/comment body; ID becomes an optional in-cycle breadcrumb (e.g., `(AC-1)`) that gracefully decays when plan doc is discarded.
...
| `specify` | REQUIREMENT → SPEC | SPEC | Derive tech contract (Outcome + conditional Invariants, Non-goals). Generic-category abstraction only. Headings `## AC-<N>: <slug>`. Enforce "what a spec is NOT" drift guard. Split episode-verifiable (Outcome) from continuous (Invariants). Research activity on REQ → SPEC edge; archive notable findings as RESEARCH entries when depth is worth preserving. |
| `design` | SPEC → DESIGN | DESIGN (+ RATIONALE, RESEARCH as needed) | Architecture diagram (Mermaid) + Decisions (`## Decision-<N>: <slug>`). Anchor non-trivial decisions to RATIONALE. Write RATIONALE entries for non-trivial decisions and RESEARCH entries along the SPEC → DESIGN research activity. Drift guard: chosen realization only; no work ordering or ops process text. |
| `plan` | DESIGN → TASK | TASK | DAG with track subgraphs + prefixed IDs. Task cards with Goal (WHAT + HOW + inline anchors), Repo, Completion, Dependencies, conditional Guidelines. Verify bidirectional mapping: every SPEC AC / Invariant maps to ≥ 1 completion criterion AND every TASK cites ≥ 1 SPEC / DESIGN / Guideline reason. Run one-deployment guardrail (DAG size threshold). |
...
- **Skill implementation**: skills are scoped in §12 but not yet written. Implementation is the next major step.
- **Dogfooding**: apply framework to an existing feature (e.g., NEWCS-3526) to validate shapes.
...
- **Progressive one-deployment sizing**: scope check currently happens at TASK time (DAG-size threshold) when most reliable, but that's after REQ/SPEC/DESIGN planning effort is sunk. Need earlier-stage heuristics (SPEC-level AC count, DESIGN-level component count) to fail oversized work sooner. Deferred.
