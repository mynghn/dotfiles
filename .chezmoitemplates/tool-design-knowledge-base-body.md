...
A personal, version-controlled store of distilled **tool-design** knowledge — how to shape the tools an agent calls: their descriptions, input/output schemas, granularity and consolidation, naming and namespacing, what they return, error messages, structured output, and tool evaluations. Use it to **retrieve** a practice on demand, **capture** a new distilled lesson, or **refresh** an entry against current state-of-the-art.

The corpus is provider-neutral and shared; this skill is a thin entry point into it. The store lives at:

- Index:   `~/.local/share/tool-design-knowledge-base/INDEX.md`
- Entries: `~/.local/share/tool-design-knowledge-base/knowledge/<practice-slug>.md`
- Helper:  `~/.local/share/tool-design-knowledge-base/scripts/td-kb`

**One mutation channel.** Read entries directly, but never hand-edit them to change the store. Every write goes through `td-kb`, which edits the chezmoi source, runs `chezmoi apply`, and commits — so each change is one recoverable commit and the store stays self-consistent. The store changes only when you invoke `td-kb`; nothing mutates it automatically or on a schedule.
...
When a reusable tool-design practice is worth keeping:

1. Pick a kebab-case `<slug>`. Check `INDEX.md` first — if the practice already exists, **refresh** it instead of adding a duplicate.
2. Author the entry and pipe it to the helper on stdin:

   ```sh
   ~/.local/share/tool-design-knowledge-base/scripts/td-kb capture <slug> <<'EOF'
   ---
   name: <slug>
...
4. Apply via the helper (it requires the entry to already exist):
   ...
   ```sh
   ~/.local/share/tool-design-knowledge-base/scripts/td-kb refresh <slug> < updated-entry.md
   ```
