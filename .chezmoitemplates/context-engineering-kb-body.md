...
A personal, version-controlled store of distilled **context-engineering** knowledge — how to manage what goes into a model's context window, why long contexts degrade, and the operations that counter it. Use it to **retrieve** a principle on demand, **capture** a new distilled lesson, or **refresh** an entry against current state-of-the-art.
...
When a reusable context-engineering insight is worth keeping:

1. Pick a kebab-case `<slug>`. Check `INDEX.md` first — if the concept already exists, **refresh** it instead of adding a duplicate.
