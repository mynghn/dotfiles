...
- [explore-execute-boundary](knowledge/explore-execute-boundary.md) — Load when deciding whether to keep working in the current session or hand off to a fresh one — at an explore→execute / plan→implement transition, after a major pivot, or when a long session should depart for an explicit new goal. The temporal/session sibling of explore-then-compact-handoff (the sub-agent/spatial one).
